 ...
 */
function hex_md5(s){ s = unicode_latin2(s); return binl2hex(core_md5(str2binl(s), s.length * chrsz));}
function b64_md5(s){ s = unicode_latin2(s); return binl2b64(core_md5(str2binl(s), s.length * chrsz));}
function str_md5(s){ s = unicode_latin2(s); return binl2str(core_md5(str2binl(s), s.length * chrsz));}
...
{
  key = unicode_latin2(key);
  data = unicode_latin2(data);
...
// Replace non-Latin2 characters by entities
function unicode_latin2(str)
{
  for (var i=0; i < str.length; i++) {
    var code = str.charCodeAt(i);
    if (unicode_latin2_map(code) == -1) {
      str = str.substr(0, i) + '&#' + code + ';' + str.substr(i + 1);
      i += code.toString().length + 2;
    }
  }
  return str;
}
...
function str2binl(str)
{
  ...
  for(var i = 0; i < str.length * chrsz; i += chrsz)
    bin[i>>5] |= (unicode_latin2_map(str.charCodeAt(i / chrsz)) & mask) << (i%32);
  return bin;
}
